...
local CHATCOLOUR_WHITE = string.char(128)
local CHATCOLOUR_YELLOW = string.char(130)
local function printMapButtonHelpText(p)
    CONS_Printf(
        p,
        ((("Current trick button is " .. CHATCOLOUR_WHITE) .. tostring(buttonToString[p.advt_trickButton or BT_BRAKE])) .. CHATCOLOUR_WHITE) .. "."
    )
    CONS_Printf(p, ((((((((((((((((((((((("Valid keys are: " .. CHATCOLOUR_YELLOW) .. "Brake") .. CHATCOLOUR_WHITE) .. ", ") .. CHATCOLOUR_YELLOW) .. "Drift") .. CHATCOLOUR_WHITE) .. ", ") .. CHATCOLOUR_YELLOW) .. "Item") .. CHATCOLOUR_WHITE) .. ", ") .. CHATCOLOUR_YELLOW) .. "Custom 1") .. CHATCOLOUR_WHITE) .. ", ") .. CHATCOLOUR_YELLOW) .. "Custom 2") .. CHATCOLOUR_WHITE) .. ", ") .. CHATCOLOUR_YELLOW) .. "Custom 3") .. CHATCOLOUR_WHITE) .. ".")
end
local function cmd_mapTrickButton(p, new_button)
    ...
    if stringToButton[new_button] == nil then
        CONS_Printf(p, ((("Invalid button name \"" .. CHATCOLOUR_YELLOW) .. new_button) .. CHATCOLOUR_WHITE) .. ".")
        return
    end
    local new_trick_button = stringToButton[new_button]
    CONS_Printf(
        p,
        ((("Trick button set to " .. CHATCOLOUR_YELLOW) .. tostring(buttonToString[new_trick_button])) .. CHATCOLOUR_WHITE) .. "."
    )
...
local function printInvertControlHelpText(p)
    CONS_Printf(p, (("Current inverted axis is " .. CHATCOLOUR_YELLOW) .. invertToString[p.advt_invertMode or INVM_NONE]) .. CHATCOLOUR_WHITE)
    CONS_Printf(p, ((((((((((((((("Valid values are: " .. CHATCOLOUR_YELLOW) .. "Off") .. CHATCOLOUR_WHITE) .. ", ") .. CHATCOLOUR_YELLOW) .. "Horizontal") .. CHATCOLOUR_WHITE) .. ", ") .. CHATCOLOUR_YELLOW) .. "Vertical") .. CHATCOLOUR_WHITE) .. ", ") .. CHATCOLOUR_YELLOW) .. "Both") .. CHATCOLOUR_WHITE) .. ".")
end
local function cmd_invertControls(p, new_invert)
    ...
    if axis_mod == nil then
        CONS_Printf(p, ((("Invalid value \"" .. CHATCOLOUR_YELLOW) .. new_invert) .. CHATCOLOUR_WHITE) .. "\".")
        printInvertControlHelpText(p)
        return
    end
    local str_add = "."
    ...
    CONS_Printf(p, (((("Inverting " .. CHATCOLOUR_YELLOW) .. invertToString[axis_mod]) .. CHATCOLOUR_WHITE) .. " axis") .. str_add)
...
if f ~= nil then
    ...
    if completed >= ADVANCETRICKS then
        ...
        print((((((("Use " .. CHATCOLOUR_YELLOW) .. "advt_resettutorial") .. CHATCOLOUR_WHITE) .. " or delete ") .. CHATCOLOUR_YELLOW) .. TUTORIAL_SAVE_FILE_NAME) .. CHATCOLOUR_WHITE)
    end
    f:close()
end
...
local function saveTutorialDone()
    ...
    f:close()
    print(((("Saved tutorial completed to " .. CHATCOLOUR_YELLOW) .. TUTORIAL_SAVE_FILE_NAME) .. CHATCOLOUR_WHITE) .. ".")
end
local function tutorialThinker()
    ...
    local at = p.advTricks
    repeat
        local ____switch97 = tutorialState
        local ____cond97 = ____switch97 == 0
        if ____cond97 then
        ...
        ____cond97 = ____cond97 or ____switch97 == 1
        if ____cond97 then
            if at.didTrick then
                tutorialState = 2
        ...
        ____cond97 = ____cond97 or ____switch97 == 2
        if ____cond97 then
        ...
        ____cond97 = ____cond97 or ____switch97 == 3
        if ____cond97 then
        ...
        ____cond97 = ____cond97 or ____switch97 == 4
        if ____cond97 then
            if at.didTrick then
                tutTricksWaited = tutTricksWaited - 1
                if tutTricksWaited <= 0 then
        ...
        ____cond97 = ____cond97 or ____switch97 == 5
        if ____cond97 then
...
local function tutorialHud(v, p)
    ...
    repeat
        local ____switch119 = tutorialState
        local trick_btn, turn_green_up, turn_green_down, up_str, down_str
        local ____cond119 = ____switch119 == 1 or ____switch119 == 2
        if ____cond119 then
        ...
        ____cond119 = ____cond119 or (____switch119 == 3 or ____switch119 == 4)
        if ____cond119 then
            turn_green_up = 0
            turn_green_down = 0
        ...
        ____cond119 = ____cond119 or (____switch119 == 5 or ____switch119 == nil)
        if ____cond119 then
            v.drawString(
                300 + add_x,
                130,
...
local function getStartPosition(p)
    local angle, _ = getTrickAngleAndShift(p)
    local btns, upbtn, downbtn = getTrickButtons(p)
    local mx = 0
    local my = 0
    local mz = 0
